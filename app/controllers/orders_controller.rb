...
class OrdersController < ApplicationController
  ...
  def create
    ...
    if @order.save
      ...
      # Створення текстового списку продуктів із кошика та підрахунок суми
      product_list = ""
      total_amount = 0
  
      current_cart.cart_items.each do |item|
        if item.product
          product_list += "#{item.product.name} (x#{item.quantity}) - #{item.product.price * item.quantity} грн, "
      ...
      # Видаляємо останню кому та пробіл
  ...
  def user_orders
    @orders = current_user.orders.order(created_at: :desc) # Вибираємо замовлення поточного користувача
  end
  private
